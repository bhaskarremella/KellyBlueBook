...
CREATE PROCEDURE [dbo].[stpGetConfiguredVehicleValue_Slim] 
...
--Region Adjustment
		...
		--Price Range
		LEFT JOIN VehiclePriceRange VPR ON FS.KBBVehicleId = VPR.KBBVehicleId AND FS.PricingTypeId = VPR.PriceTypeId
	WHERE	
...
SELECT 
     ...
     , 0
FROM @vehicle V
INNER JOIN RegionBasePrice RBP ON v.KBBVehicleID = RBP.KBBVehicleId
INNER JOIN VehicleRegion VR ON RBP.VehicleTypeRegionId = VR.VehicleTypeRegionId
